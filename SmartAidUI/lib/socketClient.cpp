    ...
    : serverAddress(serverAddress), serverPort(serverPort)
{
    ...
    connect(shareValue->sharedsocket,
...
void socketClient::sendCommand(const QJsonObject& command,
    ...
    if (!fileData.isEmpty() && FileName.size())
        {
            ...
            shareValue->sharedsocket->flush();
            shareValue->sharedsocket->write(
                QString("%1&&%2").arg(FileName).arg(fileData.size()).toUtf8());
            shareValue->sharedsocket->flush();
            shareValue->sharedsocket->write("$$$$$$$$");
            shareValue->sharedsocket->flush();
            int sendbyte = 0;
            ...
            if (QString(qbuff).contains("$$$$$$$$"))
                {
                    ...
                    if (file.open(QIODevice::WriteOnly))
                        {
                            file.write(qbuff.mid(acceptinfo[0].size() +
