...
QStringList DataBaseInterface::variantListToStringList(
                                          ...
                                          QString            DBpath)
{
    ...
    QStringList selectColumns = variantListToStringList(
        QueryAttribution["SelectColumn"].toArray().toVariantList());
    QStringList conditionColumns = variantListToStringList(
        QueryAttribution["QueryConditionColumn"].toArray().toVariantList());
    QStringList conditionValues = variantListToStringList(
        QueryAttribution["QueryConditionValue"].toArray().toVariantList());
    ...
    for (int i = 0; i < conditionValues.size(); i++)
        {
            ...
            else if (conditionColumns[i].contains("PhoneNumber") ||
                     conditionColumns[i].contains("Age"))
                                        ...
                                        QString            DBpath)
{
    ...
    // 从 QueryAttribution 中获取需要的参数
    QString     tableName = QueryAttribution["TableName"].toString();
    QStringList insertColumns = variantListToStringList(
        QueryAttribution["InsertColumn"].toArray().toVariantList());
    QStringList insertValues = variantListToStringList(
        QueryAttribution["InsertValue"].toArray().toVariantList());

    // 构建 SQL 插入语句
    QString query = "INSERT INTO " + tableName + " (" +
                    insertColumns.join(", ") + ") VALUES (";
    ...
    qDebug() << "Generated SQL query:"
             << query;  // 打印生成的 SQL 查询语句用于调试
    ...
    for (int i = 0; i < insertValues.size(); i++)
        {
            ...
            else if (insertColumns[i].contains("PhoneNumber") ||
                     insertColumns[i].contains("Age"))
                                        ...
                                        QString            DBpath)
{
    ...
    QStringList updateColumns = variantListToStringList(
        UpdateAttribution["UpdateColumn"].toArray().toVariantList());
    QVariantList updateValues =
        UpdateAttribution["UpdateValue"].toArray().toVariantList();
    QStringList conditionColumns = variantListToStringList(
        UpdateAttribution["UpdateConditionColumn"].toArray().toVariantList());
    QVariantList conditionValues =
        UpdateAttribution["UpdateConditionValue"].toArray().toVariantList();
    ...
    for (int i = 0; i < updateValues.size(); i++)
        {
            ...
            else if (updateColumns[i].contains("PhoneNumber") ||
                     updateColumns[i].contains("Age"))
