...
{
    connect(
        this, &QTcpServer::newConnection, this, &FileServer::onNewConnection);
}

QJsonObject FileServer::AnalyzeCommand(QJsonObject &CommandPack,
                                       QByteArray   FileByte)
{
    ...
    switch (HeadToInt->value(CommandHead))
        {
            case 0:
                {
                    ...
                    qlonglong UserID =
                        QVariant(CommandArgs.value("UserID")).toLongLong();
                    QString password = CommandArgs.value("Password").toString();
                    QJsonObject result =
                        RequestOperate::UserLogin(identity, UserID, password);
            ...
            case 1:
                {
                    ...
                    QString SelfAddress =
                        CommandArgs.value("SelfAddress").toString();
                    QString PhoneNumber =
                        CommandArgs.value("PhoneNumber").toString();
                    QString signature =
                        CommandArgs.value("Signature").toString();
                    return RequestOperate::UserRegister(identity,
                                                        UserID,
                                                        AvatarPic,
                                                        password,
                                                        UserName,
            ...
            case 2:
                {
                    ...
                    qlonglong UserID =
                        QVariant(CommandArgs.value("UserID")).toLongLong();
                    QByteArray &AvatarPic = FileByte;
                    return RequestOperate::UploadAvatar(
                        identity, UserID, AvatarPic);
                }
                break;
            case 3:
                {
                    QString Apartment =
                        CommandArgs.value("Apartment").toString();
            ...
            case 4:
                {
                    qlonglong PatientID =
                        QVariant(CommandArgs.value("PatientID")).toLongLong();
                    qlonglong DoctorID =
                        QVariant(CommandArgs.value("DoctorID")).toLongLong();
                    int CurTime =
                        CommandArgs.value("CurTime").toString().toInt();
                    int duration =
                        CommandArgs.value("Duration").toString().toInt();
                    return RequestOperate::Registration(
                        PatientID, DoctorID, CurTime, duration);
                }
                break;
            case 5:
                {
            ...
            case 9:
                {
                    ...
                    qlonglong UserID =
                        QVariant(CommandArgs.value("UserID")).toLongLong();
                    QString OldPassword =
                        CommandArgs.value("OldPassword").toString();
                    QString NewPassword =
                        CommandArgs.value("NewPassword").toString();
                    ...
                    QString signature =
                        CommandArgs.value("Signature").toString();
                    return RequestOperate::EditProfile(identity,
                                                       UserID,
                                                       OldPassword,
                                                       NewPassword,
                                                       Avatar,
            ...
            case 11:
                {
                    int AppointmentID =
                        CommandArgs.value("AppointmentID").toInt();
                    QString DoctorDiagnosis =
                        CommandArgs.value("DoctorDiagnosis").toString();
                    QString Perscription =
                        CommandArgs.value("Perscription").toString();
                    QString Orders = CommandArgs.value("Orders").toString();
                    return RequestOperate::EditMedicalRecord(
                        AppointmentID, DoctorDiagnosis, Perscription, Orders);
                }
                break;
            case 12:
                {
                    int       identity = CommandArgs.value("Identity").toInt();
                    qlonglong Sender =
                        QVariant(CommandArgs.value("Sender")).toLongLong();
                    qlonglong Recipient =
                        QVariant(CommandArgs.value("Recipient")).toLongLong();
                    ...
                    upFiles.close();
                    return RequestOperate::SaveFiles(
                        identity, Sender, Recipient, SendTime, FilePath);
                }
                break;
            case 13:
                {
                    ...
                    return RequestOperate::QueryHistory(
                        identity, Sender, Recipient, IsFile, SendTime, FileName);
                }
                break;
            case 98:
                {
                    qlonglong DoctorID =
                        QVariant(CommandArgs.value("DoctorID")).toLongLong();
                    QVector<int> *free =
                        RequestOperate::QueryDoctorFree(DoctorID);
                    QJsonObject result;
...
void FileServer::onNewConnection()
{
    ...
    connect(
        clientSocket, &QTcpSocket::disconnected, this, [this, clientSocket]() {
            for (qlonglong it : userList.keys())
                {
                    if (userList[it] == clientSocket)
                        {
                            qDebug()
                                << "用户" + QString::number(it) + "已断开连接";
                            userList.remove(it);
                        }
                }
        });
...
void FileServer::processCommand(QTcpSocket *clientSocket)
{
    ...
    if (QString(commandByteArray.mid(commandByteArray.size() - 11))
            .contains("$$$$END$$$$"))
        {
            ...
            if (QString(qbuff).contains("$$$$$$$$"))
                {
                    ...
                    qbuff = qbuff.mid(acceptinfo[0].size() +
                                      acceptinfo[1].size() + 16);

                    QJsonObject jsonObj =
                        QJsonDocument::fromJson(acceptinfo[0].toUtf8()).object();

                    QJsonObject returnPack =
                        this->AnalyzeCommand(jsonObj, qbuff);
                    QByteArray returnByteArray =
                        QJsonDocument(returnPack).toJson();
                    ...
                    if (HeadToInt->value(jsonObj.value("CMD").toString()) == 5)
                        {
                            QString filePath = jsonObj.value("Args")
                                                   .toObject()
                                                   .value("Avatar")
                                                   .toString();
                            QByteArray *fileData =
                                RequestOperate::DownloadFiles(filePath);
                            QString sendFileName = fileName;
                            int     fileSize = fileData->size();
                            QString fileInfo =
                                QString("%1&%2").arg(sendFileName).arg(fileSize);
                            clientSocket->write("$$$$$$$$");
                            clientSocket->flush();
                            ...
                            while (sendbyte < fileSize)
                                {
                                    QByteArray buffer = fileData->mid(
                                        sendbyte, sendbyte + 10239);
                                    sendbyte +=
                                        clientSocket->write(buffer, 10240);
                                    clientSocket->flush();
                                }
                        }
            ...
            else
                {
                    ...
                    int mayFile =
                        HeadToInt->value(commandPack->value("CMD").toString());
                    if (mayFile == 0)
                        {
                            qlonglong userid = commandPack->value("Args")
                                                   .toObject()
                                                   .value("UserID")
                                                   .toString()
                                                   .toLongLong();
...
void FileServer::sendFile(QTcpSocket    *clientSocket,
                          QJsonObject   *commandPack,
                          const QString &FilePath)
{
    QString filePath =
        commandPack->value("Args").toObject().value(FilePath).toString();
