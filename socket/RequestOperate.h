...
QJsonObject UserRegister(int         identity,
                         qlonglong   UserID,
...
QJsonObject   Registration(qlonglong PatientID,
                           qlonglong DoctorID,
                           int       SendTime,
                           int       duration);
// 接受身份和账号ID，返回个人信息
...
// 身份为1表示是医生,selfaddress表示家庭住址/所属科室
QJsonObject   EditProfile(int         identity,
                          qlonglong   UserID,
                          QString&    OldPassword,
                          QString&    NewPassword,
                          QByteArray& Avatar,
...
// 接受预约号，更新数据库，返回结果
QJsonObject   EditMedicalRecord(int      AppointmentID,
                                QString& DoctorDiagnosis,
                                QString& Perscription,
                                QString& Orders);
// 接受发送者身份，发送者ID，接受者ID和传输文件名，当前时间戳
QJsonObject   SaveFiles(int       identity,
                        qlonglong Sender,
                        qlonglong Recipient,
                        int       SendTime,
                        QString&  FilePath);
...
QJsonObject   QueryHistory(int       identity,
                           qlonglong Sender,
                           qlonglong Recipient,
                           int       IsFile,
                           int       SenfTime = 0,
...
QJsonObject   SaveMes(int       identity,
                      qlonglong Sender,
                      qlonglong Recipient,
                      int       SendTime,
                      QString&  Content);
// 接受发送者，接受者和发送时间，数据库插入语音信息
QJsonObject   VoiceToWord(int         identity,
                          qlonglong   Sender,
                          qlonglong   Recipient,
                          int         SendTime,
                          QByteArray& Content);
