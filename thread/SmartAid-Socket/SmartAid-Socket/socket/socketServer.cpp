...
class FileServer : public QTcpServer
{
   ...
   public:
	...
	explicit FileServer(QObject *parent = nullptr)
		: QTcpServer(parent), fileState(0), recvSize(0)
	{
		connect(this,
   ...
   private slots:
		...
		switch (HeadToInt->value(CommandHead))
			{
				case 0:
					{
						...
						QString password =
							CommandArgs.value("Password").toString();
						return RequestOperate::UserLogin(
							identity, UserID, password);
					}
					break;
				case 1:
					{
						...
						QString		password =
							CommandArgs.value("Password").toString();
						QString UserName =
							CommandArgs.value("UserName").toString();
				...
				case 4:
					{
						qlonglong PatientID = CommandArgs.value("PatientID")
												  .toString()
												  .toLongLong();
						qlonglong DoctorID = CommandArgs.value("DoctorID")
												 .toString()
												 .toLongLong();
						int CurTime =
				...
				case 11:
					{
						...
						return RequestOperate::EditMedicalRecord(AppointmentID,
																 DoctorDiagnosis,
																 Perscription,
																 Orders);
					}
				...
				case 12:
					{
						...
						qlonglong Recipient = CommandArgs.value("Recipient")
												  .toString()
												  .toLongLong();
						int		SendTime = CommandArgs.value("SendTime").toInt();
						QString FilePath =
							CommandArgs.value("FilePath").toString();
						QString FileName =
							CommandArgs.value("FileName").toString();
				...
				case 13:
					{
						...
						return RequestOperate::QueryHistory(identity,
															Sender,
															Recipient,
															IsFile,
															SendTime,
	...
	void onNewConnection()
	{
		...
		connect(
			clientSocket, &QTcpSocket::readyRead, this, [this, clientSocket]() {
	...
	void FileServer::incomingConnection(qintptr socketDescriptor)
	{
		...
		connect(tcpTemp,
				&QTcpSocket::readData,
				this,
				&QTcpSocket::readDataSlot);	 // 接受到数据
		connect(
			tcpTemp,
			&QTcpSocket::sockDisConnect,
			this,
			&QTcpSocket::sockDisConnectSlot);  // 断开连接并释放断开的Tcpsocket
		connect(this,
				&FileServer::sentData,
				tcpTemp,
		...
		qDebug() << "incoming connection THREAD is: "
