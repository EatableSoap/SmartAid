	...
	switch (HeadToInt->value(CommandHead))
		{
			case 0:
				{
					...
					qlonglong UserID =
						CommandArgs.value("UserID").toString().toLongLong();
			...
			case 4:
				{
					qlonglong PatientID =
						CommandArgs.value("PatientID").toString().toLongLong();
					qlonglong DoctorID =
						CommandArgs.value("DoctorID").toString().toLongLong();
					int CurTime =
			...
			case 12:
				{
					int		  identity = CommandArgs.value("Identity").toInt();
					qlonglong Sender =
						CommandArgs.value("Sender").toString().toLongLong();
					qlonglong Recipient =
						CommandArgs.value("Recipient").toString().toLongLong();
...
void Server::StartServer()
{
	...
	if (!server->listen(QHostAddress::Any, 8888))
		{
			qDebug() << "Server could not start. Error:"
					 << server->errorString();
		}
	else
		{
	...
	while (1)
		{
			connect(server, &QTcpServer::newConnection, [&]() {
				...
				childthread->start();
				clientSocket->moveToThread(childthread);
#endif
				incomingConnection(server->nextPendingConnection());
							...
							case 0:
								{
									...
									QJsonObject *CommandPack =
										MyQtJson::AnalysisByteArray(
											CommandByteArray);
									int mayFile = HeadToInt->value(
										CommandPack->value("CMD").toString());
									if (mayFile == 99)
										{
											QString FilePath =
												CommandPack->value("Args")
													.toObject()
													.value("FilePath")
													.toString();
											QByteArray *fileData =
												RequestOperate::DownloadFiles(
													FilePath);
											QString SendFileName = FilePath.mid(
									...
									if (!HasFile.contains(mayFile))
										{
											QJsonObject *ReturnPack =
												AnalyzeCommand(*CommandPack);
											QByteArray ReturnByteArray =
												QJsonDocument(*ReturnPack)
													.toJson();
				...
				thread->start();
				emit clientSocket->start(handle);
			});
		}
}
