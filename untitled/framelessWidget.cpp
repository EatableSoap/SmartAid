    ...
    , ui(new Ui::framelessWidget)
{
    ...
    action->setIcon(QIcon(":/pitcure/preview-close.png"));
    connect(ui->password,&QtMaterialAutoComplete::textEdited,this,[=]{
    ...
    connect(action,&QAction::triggered,this,[=]()mutable{
        if (stat == 0)
        {
            action->setIcon(QIcon(":/pitcure/eyes-open.png"));
            ui->password->addAction(action,QLineEdit::TrailingPosition);
            stat = 1;
...
void framelessWidget::on_Doctor_pressed()
{
    ApIasd=1;
}

void framelessWidget::on_patient_pressed()
{
    ApIasd=2;
}

void framelessWidget::on_enroll_clicked()
{
    this->close();
...
void framelessWidget::on_Login_clicked()
{
    ...
    if(ApIasd==1)
    {
        ...
        {
            ui->Tips_Column->textlable->setText("您的输入格式有误，请更改后重新尝试登陆");
            ui->Tips_Column->animationStart();
            QTimer::singleShot(1000, [this]() {
                 ui->Tips_Column->hide(); // 隐藏提示列组件
             });
        }
        else
        {
    ...
    if(ApIasd!=1&&ApIasd!=2)
    {
        Qt::Widget|Qt::WindowStaysOnTopHint;
...
void framelessWidget::on_password_textEdited(const QString &arg1)
{
    ui->password->setEchoMode(QLineEdit::Password);
}
